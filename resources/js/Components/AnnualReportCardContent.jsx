...
export default function AnnualReportCardContent({
    student,
    historicClass,
    annualResults = [],
    annualSubjectStats = {},
                ...
                <div className="border border-slate-300 text-[10px]">
                        ...
                        <div className="w-32 border-l border-slate-300 flex">
                            ...
                            <div className="flex-1 p-1.5 font-bold">
                                {student.date_of_birth ?
                                    Math.floor((new Date() - new Date(student.date_of_birth)) / 31557600000)
                                    : 'N/A'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex border-b border-slate-300">
                        ...
                        <div className="flex-1 p-1.5 font-bold text-slate-900 uppercase">
                            {historicClass
                                ...
                                : `${student.class_section?.school_class?.name || student.classSection?.schoolClass?.name || ''}${(student.class_section?.class_arm?.name || student.classSection?.classArm?.name) !== 'No arm' ? ` ${student.class_section?.class_arm?.name || student.classSection?.classArm?.name || ''}` : ''}`
                            }
                        </div>
                        <div className="w-32 border-l border-slate-300 flex">
                            <div className="w-12 bg-slate-50 p-1.5 font-bold text-slate-700 border-r border-slate-300 uppercase">Pos:</div>
                            <div className="flex-1 p-1.5 font-bold">
                                {getOrdinalSuffix(overall.position)}
                                {overall.position !== 'N/A' && ` / ${overall.total_students}`}
                                {overall.rank_scope && (
                                    <div className="text-[8px] text-slate-500 mt-0.5">({overall.rank_scope})</div>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex border-b border-slate-300">
                        ...
                        <div className="w-32 border-l border-slate-300 flex">
                            ...
                            <div className="flex-1 p-1.5 font-bold">{overall.average}</div>
                        </div>
                    </div>
                    {/* Row 5: Remark/Status */}
                    <div className="flex">
                        <div className="w-24 bg-slate-50 p-1.5 font-bold text-slate-700 border-r border-slate-300 uppercase">Status:</div>
                        <div className="flex-1 p-1.5 font-bold uppercase tracking-widest" style={{ color: primaryColor }}>
                ...
                <table className="w-full border-collapse border border-slate-900">
                    <thead>
                        <tr style={{ backgroundColor: primaryColor, color: 'white' }}>
                            ...
                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10">1st</th>
                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10">2nd</th>
                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10">3rd</th>
                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10 bg-white/10">Total</th>

                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10 text-yellow-300">Class Avg</th>
                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10">Pos</th>

                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-10">Grade</th>
                            <th className="border border-white/20 px-1 py-1 text-[9px] uppercase font-bold text-center w-16">Remark</th>
                        </tr>
                    </thead>
                    <tbody>
                        {annualResults && annualResults.length > 0 ? annualResults.map((res, index) => (
                            <tr key={index} className="text-[10px] border-b border-slate-300 hover:bg-slate-50">
                                <td className="border-r border-slate-300 px-1 py-1 text-center">{index + 1}</td>
                                <td className="border-r border-slate-300 px-2 py-1 font-bold text-slate-800 uppercase">{res.subject?.name}</td>
                                <td className="border-r border-slate-300 px-1 py-1 text-center text-slate-600 font-mono">{res.term1_score ? Number(res.term1_score).toFixed(0) : '-'}</td>
                                <td className="border-r border-slate-300 px-1 py-1 text-center text-slate-600 font-mono">{res.term2_score ? Number(res.term2_score).toFixed(0) : '-'}</td>
                                <td className="border-r border-slate-300 px-1 py-1 text-center text-slate-600 font-mono">{res.term3_score ? Number(res.term3_score).toFixed(0) : '-'}</td>
                                <td className="border-r border-slate-300 px-1 py-1 text-center font-bold bg-slate-100">{Number(res.annual_score || res.annual_avg).toFixed(1)}</td>

                                <td className="border-r border-slate-300 px-1 py-1 text-center font-medium text-slate-600">
                                    {(annualSubjectStats && res.subject_id in annualSubjectStats) ? annualSubjectStats[res.subject_id] : '-'}
                                </td>
                                <td className="border-r border-slate-300 px-1 py-1 text-center text-slate-700 whitespace-nowrap">
                                    {res.position ? getOrdinalSuffix(res.position) : '-'}
                                </td>

                                <td className="border-r border-slate-300 px-1 py-1 text-center font-bold">{res.grade?.name || res.grade}</td>
                                <td className="px-1 py-1 text-center text-[9px] uppercase font-medium">{res.grade_remark || '-'}</td>
                            </tr>
                        )) : (
                            <tr>
                                <td colSpan="10" className="text-center py-4 text-slate-500 italic">No annual results available</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            <div className="px-4 mb-4">
                <div className="mb-3">
                    <div className="text-[9px] font-bold" style={{ color: secondaryColor }}>
                        KEY TO GRADES: {grades && grades.length > 0 ? [...grades].sort((a, b) => b.min_score - a.min_score).map((g, i) => (
                ...
                <div className="border border-slate-900 rounded-lg overflow-hidden mb-6">
                    ...
                    <div className="text-[10px]">
                        <div className="flex border-b border-slate-200">
                            ...
                            <div className="flex-1 p-1.5 italic">"{promotionDecision?.form_teacher_remark || reportSettings.form_teacher_remark || 'Excellent performance.'}"</div>
                        </div>
                        <div className="flex">
                            <div className="w-24 p-1.5 font-bold bg-slate-100 border-r border-slate-200">Principal</div>
                            <div className="flex-1 p-1.5 italic">"{promotionDecision?.principal_remark || reportSettings.principal_remark || 'Excellent result.'}"</div>
                        </div>
                    </div>
                </div>
