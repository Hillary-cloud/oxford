                ...
                <div className="flex flex-wrap mt-8">
                    {links.map((link, key) => (
                        link.url === null ? (
                            ...
                            >{link.label.replace('&laquo;', 'Previous').replace('&raquo;', 'Next')}</div>
                        ) : (
                            <Link
                                key={key}
                                className={getClassName(link.active)}
                                href={link.url}
                                style={link.active ? { backgroundColor: '#4f46e5' } : {}}
                            >{link.label.replace('&laquo;', 'Previous').replace('&raquo;', 'Next')}</Link>
                        )
                    ))}
                </div>
            </div>
