                    ...
                    <form action="{{ route('terms.update', $term->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="academic_session_id" id="academic_session_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @foreach ($sessions as $session)
                                    <option value="{{ $session->id }}" {{ $term->academic_session_id == $session->id ? 'selected' : '' }}>{{ $session->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_current" value="1" {{ $term->is_current ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
